import { Button, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, styled as styledMUI } from "@mui/material";
...
export const PaymentList = () => {
    ...
    const [searchPaymentListModalIsOpen, setSearchPaymentListModalIsOpen] = useState(false);

    const fetchPaymentList = async () => {
        const paymentList: PaymentTypeList[] = await axios
            .get(`${API_URL}/paymentmethod?` + searchParams.toString()).then((res) => res.data);
        return paymentList;
    };

    const { data, isLoading, error } = useQuery({
        queryKey: [('payment' + searchParams.toString() + toggleApi) as string], queryFn: fetchPaymentList,
    });
    ...
    return (
        <>
                ...
                <StyledRightAlignDiv>
                    <StyledNormalButton
                        onClick={() =>
                            setSearchPaymentListModalIsOpen(true)
                        }
                    >
                        Search
                    </StyledNormalButton>
                                                ...
                                                <TableRow hover role="checkbox" tabIndex={-1} key={row.paymentMethodId}>
                                                    {columns.map((column) => {
                                                        if (column.id === "Button") {
                                                            ...
                                                            >
                                                                <StyledLink to={`/payment/${row.userId}`}>
                                                                    <StyledNormalButton>Detail</StyledNormalButton>
                                                                </StyledLink>
                                                                <StyledLink to={`/payment/${row.userId}`}>
                                                                    <StyledNormalButton>Approve</StyledNormalButton>
                                                                </StyledLink>
                                                                <StyledLink to={`/payment/${row.userId}`}>
                                                                    <StyledNormalButton>Reject</StyledNormalButton>
                                                                </StyledLink>
                                                            </TableCell>
                                                            );
                                                        }
